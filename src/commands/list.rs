        ...
        loop {
            ...
            match str::from_utf8(response.as_ref())? {
                "DENT" => {
                    ...
                    self.get_connection()?.read_exact(&mut file_mod)?;
                    self.get_connection()?.read_exact(&mut file_size)?;
                    self.get_connection()?.read_exact(&mut mod_time)?;
                    self.get_connection()?.read_exact(&mut name_len)?;
                    let name_len = LittleEndian::read_u32(&name_len);
                    let mut name_buf = vec![0_u8; name_len as usize];
                    self.get_connection()?.read_exact(&mut name_buf)?;
                }
                "DONE" => {
                    return Ok(());
                }
